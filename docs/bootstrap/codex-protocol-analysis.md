...
| 31 | **Elicitation** | Supported (selective mapping) | Only Pi `extension_ui_request` methods `select`, `confirm`, `input`, `editor` map to Codex `elicitation_request`. Pi's `notify`, `setStatus`, `setWidget`, `setTitle`, `set_editor_text` are informational/fire-and-forget — ignore or log, don't map to elicitation. |
